...
/// The `#[processor]` attribute macro transforms a struct definition into a complete
...
/// The macro generates:
/// 
/// 1. **processor trait implementation** - Provides compile-time info about buffer count
/// 2. **ProcessorName methods** - Type-safe buffer handle constructors
...
// Example Filter component with struct processor:
...
impl SawOsc {
    pub const fn named(name: &'static str) -> ProcessorName<SawOsc> {
        ProcessorName::<SawOsc> { name, _phantom: PhantomData }
    }
}

impl ProcessorName<SawOsc> {
    pub fn audio_out(self) -> BufferHandle<Output> {
